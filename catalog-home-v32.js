(() => {
  ...
  const mainNav=document.querySelector('.main-nav'); if(mainNav&&!mainNav.querySelector('.nav-perspectives')){const link=document.createElement('a'); link.className='nav-perspectives'; link.href='perspectivas.html'; link.textContent='Perspectivas'; const contactLink=[...mainNav.querySelectorAll('a')].find((item)=>item.getAttribute('href')==='#contacto'); mainNav.insertBefore(link,contactLink||null);}
  ...
  const setVersion=()=>{const versionLabel=[...document.querySelectorAll('.footer-bottom span')].find((item)=>item.textContent.includes('Web demostrativa')); if(versionLabel)versionLabel.textContent='Web demostrativa v4.3.0';};
