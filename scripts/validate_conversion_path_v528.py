...
def main() -> int:
    ...
    readiness_region = '<div class="contact-readiness-items-v528" tabindex="0" role="region" aria-label="Datos mínimos de la solicitud">'
    ...
    require(text.count(readiness_region) == 1, "la región de datos mínimos debe ser focable y etiquetada")
    require(text.count(synthesis_deck) == 1, "la síntesis debe conservar dl nativo, foco y etiqueta")
    require(text.count(brief_deck) == 1, "el brief debe conservar dl nativo, foco y etiqueta")
    require('contact-synthesis-grid-v523" tabindex="0" role="region"' not in text, "la síntesis no puede reemplazar la semántica nativa de dl")
    require('contact-brief-grid-v523" tabindex="0" role="region"' not in text, "el brief no puede reemplazar la semántica nativa de dl")
    ...
    print("CONVERSION PATH V5.28 OK: contacto adelantado, un solo formulario, decks focables, semántica dl preservada, contraste reforzado y profundidad íntegra.")
    return 0
