...
def validate():
    ...
    if home_links_script.exists():
        ...
        for marker in ("Auditoría Jurídica Empresarial Integral","Programa de Gobernanza Jurídica y Uso Responsable de IA","Proyecto Regulado Jurídicamente Estructurado","Programa de Datos, Consumidor y Canales Digitales"):
            if marker not in home_links_text: errors.append(f"catalog-home-v32.js no incorpora producto v4.1: {marker}")
    if not HTML_FILES: errors.append("No se encontraron archivos HTML"); return errors
    parsed_pages={}
    ...
    if perspective_count!=6: errors.append(f"Se esperaban 6 perspectivas profundas y se encontraron {perspective_count}")
    sector_count=sum(1 for page in parsed_pages if page.relative_to(ROOT.resolve()).as_posix() in SECTOR_PAGES)
    if sector_count!=8: errors.append(f"Se esperaban 8 páginas sectoriales y se encontraron {sector_count}")
    for page,parser in parsed_pages.items():
        relative=page.relative_to(ROOT.resolve()).as_posix(); own_targets=set(parser.ids)|set(parser.routes)
        for _tag,reference,line in parser.references:
            ...
            except ValueError as exc: errors.append(f"{relative}:{line}: {reference!r}: {exc}"); continue
            if target is None: continue
            if not target.exists(): errors.append(f"{relative}:{line}: recurso inexistente {reference!r}"); continue
