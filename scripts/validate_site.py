...
REQUIRED_FILES = {
    ...
    "styles.css", "site-v3.css", "clarity-v31.css", "catalog-v32.css", "enhancements.css", "experiencia.css", "firma.css", "perspectivas.css", "sectores.css", "visual-v39.css",
    "site-v3.js", "catalog-v32.js", "catalog-home-v32.js", "demo.js", "experiencia.js", "visual-v39.js",
...
class SiteParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "html" and values.get("lang", "").strip():
            self.has_lang = True
        if tag == "body" and values.get("data-catalog-id"):
            self.catalog_id = values["data-catalog-id"]
...
def validate() -> list[str]:
    ...
    for page in HTML_FILES:
        ...
        if duplicates:
            errors.append(f"{relative}: IDs duplicados: {', '.join(sorted(duplicates))}")
        if not parser.has_lang:
            errors.append(f"{relative}: falta atributo lang en <html>")
        if not parser.has_charset:
            errors.append(f"{relative}: falta meta charset")
        if not parser.has_viewport:
            errors.append(f"{relative}: falta meta viewport")
        if not parser.has_title:
            errors.append(f"{relative}: falta un título no vacío")
        if parser.images_without_alt:
            errors.append(f"{relative}: imágenes sin alt en líneas {', '.join(map(str, parser.images_without_alt))}")
        ...
        for _tag, reference, line in parser.references:
            if reference.startswith("#"):
                ...
                if anchor and anchor not in own_targets:
                    errors.append(f"{relative}:{line}: ancla o ruta inexistente #{anchor}")
                continue
            try:
            ...
            if fragment and target.suffix.lower() == ".html":
                ...
                if target_parser:
                    ...
                    if fragment not in targets:
                        errors.append(
                            f"{relative}:{line}: ancla o ruta #{fragment} inexistente en {target.relative_to(ROOT.resolve())}"
                        )
    return errors
