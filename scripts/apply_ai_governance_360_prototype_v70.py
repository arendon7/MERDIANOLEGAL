#!/usr/bin/env python3
"""Materialize the AI Governance 360 v7 prototype on existing canonical surfaces."""
...
def render_route(route: dict) -> str:
    ...
    for stage in route["stages"]:
        cards.append(
            '<article class="v6-route-option" data-v7-ai-governance-stage="%s">'
    ...
    return (
        f"{ROUTE_START}\n"
        '<section class="v6-section" id="v7-ai-governance-360" aria-labelledby="v7-ai-governance-360-title" data-v7-ai-governance-360="route">'
        '<div class="v6-container"><div class="v6-section-head">'
        f'<p class="v6-eyebrow">{esc(route["eyebrow"])}</p>'
        f'<h2 class="v6-heading" id="v7-ai-governance-360-title">{esc(route["title"])}</h2>'
        f'<p class="v6-lead">{esc(route["lead"])}</p></div>'
        '<div class="v6-route-list" aria-label="Etapas de AI Governance 360">' + ''.join(cards) + '</div>'
...
def render_deep(item: dict) -> str:
    ...
    return (
        f"{start}\n"
        f'<section class="v6-section v6-result" id="v7-{esc(item["id"])}" aria-labelledby="v7-{esc(item["id"])}-title" data-v7-ai-governance-deep="{esc(item["id"])}">'
        ...
        '<div class="v6-container"><div class="v6-pricing-note" data-v7-ai-governance-continuity="true"><div>'
...
def expected_route(route: dict) -> tuple[Path, str]:
    ...
    if not target.exists():
        raise SystemExit(f"Missing AI Governance route target: {route['target']}")
...
def expected_deep(item: dict) -> tuple[Path, str]:
    ...
    if not target.exists():
        raise SystemExit(f"Missing AI Governance deep target: {item['target']}")
    start, end = deep_markers(item)
...
def main() -> None:
    ...
    for target, expected in surfaces:
        ...
        if args.check:
            if current != expected:
                raise SystemExit(f"AI Governance 360 v7 drift detected in {target.relative_to(ROOT)}")
        elif current != expected:
            target.write_text(expected, encoding="utf-8")
            changed.append(str(target.relative_to(ROOT)))
    if args.check:
        print("AI Governance 360 v7 prototype --check: PASS (3/3)")
    elif changed:
        print("Materialized AI Governance 360 v7 prototype: " + ", ".join(changed))
    else:
        print("AI Governance 360 v7 prototype already materialized: 3/3")
