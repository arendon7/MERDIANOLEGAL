...
PAGES = [
    ...
    ("productos/diagnostico-juridico-empresarial.html", "product-diagnostic", "product", "Auditoría Jurídica Empresarial Integral", "Auditoría jurídica empresarial integral para revisar estructura, contratos, obligaciones, activos y controles, priorizar hallazgos y definir un plan de regularización de 90 días.", "Diagnóstico jurídico"),
    ...
    ("productos/programa-gobernanza-ia.html", "product-ai", "product", "Programa de Gobernanza Jurídica y Uso Responsable de IA", "Programa para inventariar casos de uso de IA, clasificar riesgos y definir roles, controles, proveedores, supervisión humana, evidencia e incidentes.", "Gobernanza de IA"),
    ("productos/proyecto-regulado-estructurado.html", "product-regulated", "product", "Proyecto Regulado Jurídicamente Estructurado", "Programa de viabilidad, habilitación y control jurídico para ordenar territorio, autoridades, permisos, contratos, condiciones precedentes y obligaciones de un proyecto regulado.", "Proyecto regulado"),
    ...
    ("productos/proteccion-datos-consumidor.html", "product-data-consumer", "product", "Programa de Datos, Consumidor y Canales Digitales", "Programa operativo de privacidad, protección al consumidor y canales digitales para convertir políticas en inventarios, términos, contratos, procedimientos, responsables y evidencia.", "Legal Operations"),
]
...
def render(path: str, catalog_id: str, page_type: str, title: str, description: str, need: str) -> str:
    ...
    schema = {"@context": "https://schema.org", "@type": schema_type, "name": title, "description": description, "url": canonical, "provider": {"@type": "LegalService", "name": "Meridiano Legal", "url": BASE_URL, "areaServed": "Colombia"}, "category": kind}
    breadcrumb = {"@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": [{"@type": "ListItem", "position": 1, "name": "Inicio", "item": BASE_URL}, {"@type": "ListItem", "position": 2, "name": section, "item": f"{BASE_URL}#{anchor}"}, {"@type": "ListItem", "position": 3, "name": title, "item": canonical}]}
...
def main() -> None:
    generated = 0
    for page in PAGES:
        # v4.1 is a product-catalog release. Professional services remain frozen
        # at the last approved public version until their dedicated editorial cycle.
        if page[2] != "product":
            continue
        path = ROOT / page[0]
        ...
        generated += 1
    print(f"Generadas {generated} fichas de producto canónicas; servicios preservados.")
