...
def main() -> int:
    ...
    if semver(version) >= (5, 23, 0):
        ...
        if current >= (6, 0, 0):
            ...
            if result:
                return result
            result = apply_experience_solutions_v60()
            if result:
                return result
            # v6 reemplaza el main de Home. Reaplicamos v5.29 desde su fuente
            # canónica y lo posicionamos fuera del disclosure comercial para que
            # confianza siga visible antes del contacto en cada pasada.
